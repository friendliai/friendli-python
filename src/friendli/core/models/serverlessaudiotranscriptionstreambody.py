...
class ServerlessAudioTranscriptionStreamBodyFileTypedDict(TypedDict):
    file_name: str
    content: Union[bytes, IO[bytes], io.IOBase]
    content_type: NotRequired[str]


class ServerlessAudioTranscriptionStreamBodyFile(BaseModel):
    file_name: Annotated[
...
ServerlessAudioTranscriptionStreamBodyChunkingStrategyTypedDict = TypeAliasType(
...
ServerlessAudioTranscriptionStreamBodyChunkingStrategy = TypeAliasType(
...
class ServerlessAudioTranscriptionStreamBodyTypedDict(TypedDict):
    model: str
    "Code of the model to use. See [available model list](https://friendli.ai/docs/guides/model-apis/pricing#billing-methods)."
    file: ServerlessAudioTranscriptionStreamBodyFileTypedDict
    ...
    chunking_strategy: NotRequired[
        Nullable[ServerlessAudioTranscriptionStreamBodyChunkingStrategyTypedDict]
    ]
    'Controls how the audio is cut into chunks. When set to `\\"auto\\"`, the server first normalizes loudness and then uses voice activity detection (VAD) to choose boundaries. `server_vad` object can be provided to tweak VAD detection parameters manually. If unset, the audio is transcribed as a single block.'
...
class ServerlessAudioTranscriptionStreamBody(BaseModel):
    ...
    chunking_strategy: Annotated[
        OptionalNullable[ServerlessAudioTranscriptionStreamBodyChunkingStrategy],
