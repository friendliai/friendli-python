...
class ContainerAudioTranscriptionStreamBodyFileTypedDict(TypedDict):
    file_name: str
    content: Union[bytes, IO[bytes], io.IOBase]
    content_type: NotRequired[str]


class ContainerAudioTranscriptionStreamBodyFile(BaseModel):
    file_name: Annotated[
...
ContainerAudioTranscriptionStreamBodyChunkingStrategyTypedDict = TypeAliasType(
...
ContainerAudioTranscriptionStreamBodyChunkingStrategy = TypeAliasType(
...
class ContainerAudioTranscriptionStreamBodyTypedDict(TypedDict):
    file: ContainerAudioTranscriptionStreamBodyFileTypedDict
    ...
    chunking_strategy: NotRequired[
        Nullable[ContainerAudioTranscriptionStreamBodyChunkingStrategyTypedDict]
    ]
    'Controls how the audio is cut into chunks. When set to `\\"auto\\"`, the server first normalizes loudness and then uses voice activity detection (VAD) to choose boundaries. `server_vad` object can be provided to tweak VAD detection parameters manually. If unset, the audio is transcribed as a single block.'
...
class ContainerAudioTranscriptionStreamBody(BaseModel):
    file: Annotated[
        ContainerAudioTranscriptionStreamBodyFile,
        FieldMetadata(multipart=MultipartFormMetadata(file=True)),
    ]
    "The audio file object (not file name) to transcribe, in one of these formats: mp3, wav, flac, ogg, and many other standard audio formats."
    model: Annotated[OptionalNullable[str], FieldMetadata(multipart=True)] = UNSET
    "Routes the request to a specific adapter."
    chunking_strategy: Annotated[
        OptionalNullable[ContainerAudioTranscriptionStreamBodyChunkingStrategy],
    ...
    def serialize_model(self, handler):
        optional_fields = set(
            ["model", "chunking_strategy", "language", "stream", "temperature"]
        )
        nullable_fields = set(["model", "chunking_strategy", "language", "temperature"])
        serialized = handler(self)
        m = {}
        for n, f in type(self).model_fields.items():
            k = f.alias or n
