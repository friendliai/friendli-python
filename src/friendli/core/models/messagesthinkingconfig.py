"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from friendli.core.types import (
    BaseModel,
    Nullable,
    OptionalNullable,
    UNSET,
    UNSET_SENTINEL,
)
from pydantic import model_serializer
from typing import Literal
from typing_extensions import NotRequired, TypedDict

MessagesThinkingConfigType = Literal["enabled", "disabled", "adaptive"]
"Reasoning mode. `enabled` forces reasoning, `disabled` suppresses it, and `adaptive` lets the system decide."


class MessagesThinkingConfigTypedDict(TypedDict):
    type: MessagesThinkingConfigType
    "Reasoning mode. `enabled` forces reasoning, `disabled` suppresses it, and `adaptive` lets the system decide."
    budget_tokens: NotRequired[Nullable[int]]
    "Reasoning token budget. Required when `type=enabled`; must not be provided when `type=disabled` or `type=adaptive`. If `max_tokens` is set, this must be smaller than `max_tokens`."


class MessagesThinkingConfig(BaseModel):
    type: MessagesThinkingConfigType
    "Reasoning mode. `enabled` forces reasoning, `disabled` suppresses it, and `adaptive` lets the system decide."
    budget_tokens: OptionalNullable[int] = UNSET
    "Reasoning token budget. Required when `type=enabled`; must not be provided when `type=disabled` or `type=adaptive`. If `max_tokens` is set, this must be smaller than `max_tokens`."

    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(["budget_tokens"])
        nullable_fields = set(["budget_tokens"])
        serialized = handler(self)
        m = {}
        for n, f in type(self).model_fields.items():
            k = f.alias or n
            val = serialized.get(k, serialized.get(n))
            is_nullable_and_explicitly_set = (
                k in nullable_fields and self.__pydantic_fields_set__.intersection({n})
            )
            if val != UNSET_SENTINEL:
                if (
                    val is not None
                    or k not in optional_fields
                    or is_nullable_and_explicitly_set
                ):
                    m[k] = val
        return m
