...
DedicatedTextClassificationBodyInputTypedDict = TypeAliasType(
    "DedicatedTextClassificationBodyInputTypedDict", Union[str, List[str]]
)
...
DedicatedTextClassificationBodyInput = TypeAliasType(
    "DedicatedTextClassificationBodyInput", Union[str, List[str]]
)
"Input text to classify, encoded as a string or array of strings. To classify multiple inputs in a single request, pass an array of strings.\n\nEither `input` or `tokens` field is required.\n"


class DedicatedTextClassificationBodyTypedDict(TypedDict):
    model: str
    ...
    input: NotRequired[Nullable[DedicatedTextClassificationBodyInputTypedDict]]
    "Input text to classify, encoded as a string or array of strings. To classify multiple inputs in a single request, pass an array of strings.\n\n    Either `input` or `tokens` field is required.\n    "
    tokens: NotRequired[Nullable[List[int]]]
    "The tokenized prompt (i.e., input tokens).\n\n    Either `input` or `tokens` field is required.\n    "


class DedicatedTextClassificationBody(BaseModel):
    model: str
    'ID of target endpoint. If you want to send request to specific adapter, use the format \\"YOUR_ENDPOINT_ID:YOUR_ADAPTER_ROUTE\\". Otherwise, you can just use \\"YOUR_ENDPOINT_ID\\" alone.'
    input: OptionalNullable[DedicatedTextClassificationBodyInput] = UNSET
    "Input text to classify, encoded as a string or array of strings. To classify multiple inputs in a single request, pass an array of strings.\n\n    Either `input` or `tokens` field is required.\n    "
    ...
    def serialize_model(self, handler):
        optional_fields = set(["input", "tokens"])
        nullable_fields = set(["input", "tokens"])
